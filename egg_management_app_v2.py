    ...
    with col2:
        ...
        with tab1:
            ...
            with st.form("login_form", clear_on_submit=False):
                ...
                username = st.text_input("Username", key="login_username", autocomplete="username", placeholder="Enter username")
                password = st.text_input("Password", key="login_password", type="password", autocomplete="current-password", placeholder="Enter password")

                # CAPTCHA answer inside form - this is the critical fix
                captcha_answer = st.text_input("CAPTCHA Answer", key="captcha_answer_input", max_chars=3, placeholder="?")

                # Store captcha session in a hidden field or session state
                # We use a hidden text input to preserve the session ID through form submission
                st.text_input("Session", value=captcha_session, key="captcha_session_field", label_visibility="collapsed")

                submitted = st.form_submit_button("🔐 Login Securely", type="primary")

            # Process form submission OUTSIDE the form
            if submitted:
                # Get the captcha session from the hidden field (preserved through form)
                current_session = st.session_state.get('captcha_session_field', captcha_session)
                current_answer = captcha_answer.strip() if captcha_answer else ""

                if not current_answer:
                    st.error("⚠️ Please solve the math problem before logging in.")
                    # Don't refresh CAPTCHA, let user try again with same problem
                else:
                    ip = "127.0.0.1"
                    ua = "Streamlit App"

...
class MathCaptcha:
    """
    Math CAPTCHA that stores the answer directly in st.session_state.
    This survives Streamlit reruns reliably — no in-memory dict needed.
    """

    def generate(self, session_id: str) -> Dict[str, Any]:
        """Generate a new math problem and store answer in session_state"""
        ...
        # Store answer in session_state — survives all reruns
        st.session_state['_captcha_answer'] = answer
        st.session_state['_captcha_question'] = question
        st.session_state['_captcha_attempts'] = 0
    ...
    def verify(self, session_id: str, user_answer: str) -> Dict[str, Any]:
        """Verify answer from session_state — no external store needed"""
        correct = st.session_state.get('_captcha_answer')

        if correct is None:
            return {'success': False, 'error': 'CAPTCHA expired. Please refresh the page.'}

        attempts = st.session_state.get('_captcha_attempts', 0) + 1
        st.session_state['_captcha_attempts'] = attempts

        if attempts > 5:
            # Clear so a new one generates on next render
            st.session_state.pop('_captcha_answer', None)
            st.session_state.pop('_captcha_question', None)
            return {'success': False, 'error': 'Too many attempts. CAPTCHA refreshed.'}

        try:
            user_int = int(str(user_answer).strip())
        except (ValueError, AttributeError):
            return {'success': False, 'error': 'Please enter a whole number.'}
        ...
        return {'success': False, 'error': f'Wrong answer. Try again. ({attempts}/5)'}

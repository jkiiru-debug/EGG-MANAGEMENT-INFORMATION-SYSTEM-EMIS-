...
class SecurityConfig:
    ...
    # Password Hashing
    BCRYPT_ROUNDS = 12
    BCRYPT_PEPPER = os.getenv('BCRYPT_PEPPER', secrets.token_hex(16))
...
class CryptoManager:
    ...
    def verify_password(password: str, hashed: str) -> bool:
        if hashed.startswith("pbkdf2:"):
            ...
            return CryptoManager._secure_compare(hashed, f"pbkdf2:{salt}:{expected}")
        if BCRYPT_AVAILABLE:
            peppered = password + SecurityConfig.BCRYPT_PEPPER
...
class DatabaseManager:
    """SQLite database with multi-shop support"""
...
def render_math_captcha(captcha: MathCaptcha, form_context: bool = False):
    """Render mathematical CAPTCHA widget

    Args:
        captcha: MathCaptcha instance
        form_context: If True, renders inside a form (no button allowed)
    """
    # Initialize or refresh CAPTCHA session
    if 'captcha_session' not in st.session_state or st.session_state.get('refresh_captcha'):
        ...
        # Clear old captcha data when session changes
    ...
    with col1:
        st.markdown(f"""
        <div style="background: #f0f2f6; padding: 10px; border-radius: 8px; text-align: center; font-size: 1.2rem; font-weight: bold; color: #1f4e78;">
            {captcha_data['question']}
        </div>
        """, unsafe_allow_html=True)
    with col2:
        # Use session state to preserve answer across reruns
        answer = st.text_input("Your Answer", max_chars=3, key="captcha_answer", placeholder="?")

    return answer, st.session_state['captcha_session']
...
def login_page(auth_service: AuthService, captcha: MathCaptcha):
    ...
    with col2:
        ...
        with tab1:
            # CAPTCHA and refresh button OUTSIDE form (widgets can't be in forms with buttons)
            captcha_answer, captcha_session = render_math_captcha(captcha)

            col_c1, col_c2 = st.columns([4, 1])
            with col_c1:
                st.caption("🧮 Security Verification - Solve the math problem above")
            with col_c2:
                if st.button("🔄 Refresh", help="Get new math problem"):
                    st.session_state['refresh_captcha'] = True
                    st.rerun()

            with st.form("login_form", clear_on_submit=False):
                st.subheader("Secure Login")
                username = st.text_input("Username", autocomplete="username", placeholder="Enter username")
                password = st.text_input("Password", type="password", autocomplete="current-password", placeholder="Enter password")

                # CAPTCHA answer MUST be inside the form to be submitted with it
                captcha_answer_in_form = st.text_input("CAPTCHA Answer", max_chars=3, key="captcha_answer_form", placeholder="Enter answer from above")

                # Hidden field for captcha session
                st.text_input("CAPTCHA Session", value=captcha_session, key="captcha_session_hidden", label_visibility="collapsed")

                submitted = st.form_submit_button("🔐 Login Securely", type="primary")

            # Handle form submission OUTSIDE the form block
            if submitted:
                # Use the form's captcha answer (not the external one)
                final_captcha_answer = captcha_answer_in_form if captcha_answer_in_form else captcha_answer

                if not final_captcha_answer:
                    st.error("⚠️ Please solve the math problem above.")
                    st.session_state['refresh_captcha'] = True
                    st.rerun()
                else:
                    ...
                    result = auth_service.authenticate(username, password, final_captcha_answer, captcha_session, ip, ua)

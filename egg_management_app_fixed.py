...
def render_math_captcha(captcha: MathCaptcha):
    ...
    if 'captcha_session' not in st.session_state:
        st.session_state['captcha_session'] = secrets.token_hex(16)

    if 'captcha_data' not in st.session_state or st.session_state.get('refresh_captcha'):
    ...
    with col3:
        # Button is OUTSIDE the form - safe to use
        pass  # Button handled outside form context
...
def login_page(auth_service: AuthService, captcha: MathCaptcha):
    ...
    with col2:
        ...
        with tab1:
            ...
            if submitted:
                if not captcha_answer:
                    ...
                    result = auth_service.authenticate(username, password, captcha_answer, captcha_session, ip, ua)
        ...
        with tab2:
            ...
            # Handle form submission OUTSIDE the form block - CORRECT
            if submit_reg:
                if all([new_user, new_pass, new_email, new_name]):
                    if len(new_pass) < SecurityConfig.PASSWORD_MIN_LENGTH:
                        ...
                        success, msg = db.create_user(new_user, new_pass, new_email, new_name, referred, selected_shop_id)
                        if success:
                            st.success(msg)
